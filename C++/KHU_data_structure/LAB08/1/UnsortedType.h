...
{	
	//return 0; // return 문이 처음에 오면 항상 0이 return되고 종료됨.
	if (list != NULL)
...
ItemType UnsortedType<ItemType>::SumSquares_b(NodeType<ItemType>* list) 
{
	...
	while (list != NULL)
	{
		sum = list->info * list->info + sum; // list->info의 제곱을 더해줘야 함.
		list = list->next;
	}
	return sum;
}
...
template <class ItemType>
ItemType UnsortedType<ItemType>::SumSquares_c(NodeType<ItemType>* list) //정상
{	
	if (list == NULL)
		return 0;
	else
...
template <class ItemType>
ItemType UnsortedType<ItemType>::SumSquares_d(NodeType<ItemType>* list) //정상
{
	if (list->next == NULL)
...
{	
	if (list->next == NULL)
		return list->info * list->info; // 0이 아니라 제곱항을 return해줘야 함.
	else
		//return SumSquares_e(list->next)*SumSquares_e(list->next); 
		// SumSquares_e(list->next)값의 제곱이 아니라, list->info의 제곱에다가 다음 요소의 sumsquare 값을 더해준 값을 리턴해야 함.
